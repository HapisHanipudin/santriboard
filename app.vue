<template>
  <NuxtLoadingIndicator color="#3e62fe" :height="4" />
  <Head>
    <Link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css" />
  </Head>
  <UApp>
    <div class="flex justify-center w-full font-sans p-4">
      <div class="container min-h-screen grid grid-cols-1 lg:grid-cols-12 gap-6">
        <button @click="changeCanvas()" class="lg:hidden z-40 p-4 rounded-r-full bg-primary fixed bottom-0 left-0 flex items-center justify-center -translate-y-8"><UIcon name="solar:hamburger-menu-bold" size="30" /></button>
        <div :class="{ 'max-lg:-translate-x-full': mobileCanvas === false }" class="col-span-3 max-lg:fixed transition-transform duration-150 ease-in-out max-lg:top-0 max-lg:left-0 max-lg:h-dvh z-50">
          <Sidenav class="z-50 lg:sticky top-10 left-0" />
        </div>
        ...
        <div class="lg:col-span-9 max-w-full">
          <NuxtPage />
        </div>
      </div>
    </div>
