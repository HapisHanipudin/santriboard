<template>
  ...
  <UApp>
    <button @click="changeCanvas()" class="lg:hidden p-4 rounded-r-full bg-blue-600 fixed top-0 left-0 flex items-center justify-center translate-y-8"><UIcon name="solar:hamburger-menu-bold" size="30" /></button>
    <div class="flex justify-center w-full font-sans p-4">
      <div class="container min-h-screen grid lg:grid-cols-12 gap-6">
        <div :class="{ 'max-lg:-translate-x-full': mobileCanvas === false }" class="col-span-3 max-lg:fixed transition-transform duration-150 ease-in-out max-lg:top-0 max-lg:left-0 max-lg:h-dvh z-50">
          <button @click="changeCanvas()" class="lg:hidden absolute top-0 left-0 translate-x-4 translate-y-6"><UIcon name="maki:cross" size="20" /></button>
          <Sidenav />
        </div>
