    ...
    >
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div class="flex flex-col gap-3">
          <UFormField name="itTopic" label="Topik IT">
            <UInput v-model="state.itTopic" class="w-full rounded-xl" size="xl" placeholder="Topik IT" />
          </UFormField>
        ...
        <div class="flex flex-col gap-3">
          <UFormField name="completion" label="Tingkat ketuntasan">
            <UInputNumber
              :format-options="{
                style: 'percent',
              }"
              ...
              size="xl"
              placeholder="Tingkat Ketuntasan"
            />
          </UFormField>
...
const onSubmit = async (input: any) => {
  try {
    const data = await $fetch("/api/admin/teacher/penilaian/it", {
      method: "POST",
      body: { ...input, completion: input.completion * 100 },
    });
...
const validate = (state: any): FormError[] => {
  const errors = [];
  if (!state.itTopic) errors.push({ name: "itTopic", message: "Required" });
  return errors;
};
