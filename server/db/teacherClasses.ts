...
export async function assignTeacherToClass(classId: string, teacherId: string, semesterId: string) {
...
export const assignTeachersToClass = async ({ teachers, classId, semesterId }: { teachers: string[]; classId: string; semesterId: string }) => {
  return await prisma.teacherClasses.createMany({
...
export async function updateTeacherClassAssignment(classId: string, teacherId: string, semesterId: string, dataToUpdate: Partial<Omit<TeacherClasses, "classId" | "teacherId" | "semesterId">>) {
