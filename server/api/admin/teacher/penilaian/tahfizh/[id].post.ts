...
export default defineEventHandler(async (event) => {
  ...
  const mistakeCount =
    typeof body.mistakeCount === "number" ? body.mistakeCount : 0;
  const repeatedCount =
    typeof body.repeatedCount === "number" ? body.repeatedCount : 0;
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        detail: {
          id: assessment.id,
          page: assessment.detail?.page ?? null,
          pageCount: assessment.detail?.pageCount ?? null,
        } as {
          id: number;
          page: string | null;
